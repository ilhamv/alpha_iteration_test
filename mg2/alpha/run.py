...
delta_list = np.linspace(0.22,1.48,500)
alpha0  = []
rat12    = []
...
delta_list_mc = np.linspace(0.22,1.48,10)
delta_list_mc = np.append(delta_list_mc,1)
alpha_mc = []
rat12_mc = []

for delta in delta_list_mc:
    beta1 = delta/4
    beta2 = delta/8
    beta  = beta1 + beta2
    ...
    uo2_xsdata.set_inverse_velocity([1/v1, 1/v2], temperature=294.)

    uo2_xsdata.set_total([SigmaT1, SigmaT2], temperature=294.)
    uo2_xsdata.set_absorption([SigmaT1-1, SigmaT2-1], temperature=294.)
    scatter_matrix = [[[SigmaS11], [SigmaS12]], [[SigmaS21], [SigmaS22]]]
    uo2_xsdata.set_scatter_matrix(scatter_matrix, temperature=294.)
    uo2_xsdata.set_decay_rate([lam1, lam2], temperature=294.)

    nu_p  = (1.0 - beta)*nu
    nu_d1 = beta1*nu
    nu_d2 = beta2*nu
    uo2_xsdata.set_prompt_nu_fission([nu_p*SigmaF1, nu_p*SigmaF2], temperature=294.)
    uo2_xsdata.set_delayed_nu_fission([[nu_d1*SigmaF1, nu_d1*SigmaF2],
                                       [nu_d2*SigmaF1, nu_d2*SigmaF2]], temperature=294.)
    uo2_xsdata.set_chi_prompt([chiP1, chiP2], temperature=294.)
    uo2_xsdata.set_chi_delayed([[chiD11, chiD12], [chiD21, chiD22]], temperature=294.)
    ...
    materials['uo2'].set_density('macro', 1.)
...
with h5py.File('result.h5', 'w') as hdf:
    hdf.create_dataset('delta_list', data=delta_list)
    hdf.create_dataset('delta_list_mc', data=delta_list_mc)
